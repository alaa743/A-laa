...
(function () {
    ...
    /* ---------------------------------------------------------
       9a. Scrambled portrait — the server only ever sends a
           tile-shuffled JPEG; it is put back together here on a
           <canvas>. Generated by _tools/scramble.html.
       --------------------------------------------------------- */
    // Must stay identical to tileOrder() in _tools/scramble.html
    ...
    $$('canvas[data-tiles]').forEach(function (canvas) {
        var cols = parseInt(canvas.dataset.cols, 10);
        var rows = parseInt(canvas.dataset.rows, 10);
        var seed = parseInt(canvas.dataset.seed, 10);
        var gutter = parseInt(canvas.dataset.gutter, 10) || 0;
        var img = new Image();

        img.onload = function () {
            var ctx = canvas.getContext('2d');
            var tw = canvas.width / cols, th = canvas.height / rows;   // visible tile
            var cw = tw + 2 * gutter, ch = th + 2 * gutter;           // stored cell

            // Copy each cell's inner tile back to its original slot; the gutter
            // absorbs JPEG bleed between shuffled neighbours and is discarded.
        ...
        img.src = canvas.dataset.tiles;
    });

    /* ---------------------------------------------------------
       9b. Image protection — block the context menu, drag and
           long-press save on protected images. A deterrent only:
           anything a browser displays can still be screenshotted.
       --------------------------------------------------------- */
    $$('.protected').forEach(function (el) {
        el.addEventListener('contextmenu', function (e) {
            e.preventDefault();
            toast('This image is protected © A’laa Hany');
        });
